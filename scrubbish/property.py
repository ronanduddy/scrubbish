...
class Property(Page):
    ...
    def to_h(self):
        ...
        result["Rates"] = (
            0.0 if (rates_text == "?" or rates_text == "") else float(rates_text)
        )
        ...
        result["Agent"] = (
            self.content().find_all("p", "enquiry-org")[0].text.strip("\n")
        )
        result["Agent Phone"] = self.content().find_all(
            "a", attrs={"data-office-phone": True}
        )[0]["data-office-phone"]
        ...
        result["Features"] = (
            self.content().find_all("div", "prop-descr-text")[0].text.replace("\n", " ")
        )
